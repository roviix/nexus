...
- **按需「不封顶」写成了 Spending 页的 Fixed $0。** 仪表盘把 No Limit 编码成 `hardLimit=2147483647`
  （i32 最大值）；省略这个字段时 Connect 默认成 0，上游当成 $0 上限。已经配过的号再跑一遍配置即可改过来。
- 批量导入勾「顺手配置」的步骤文案仍写成「先换 session 再铸 Key」——实际顺序早就改了，文案跟上。

## [0.7.1] — 2026-09-20

### 新增

- **入库自动配置多了一步「开数据保留策略」**：打开 Fable 5 的数据保留同意（仪表盘 Privacy 那格，
  走 `set-user-no-zdr-model-consent`）。幂等，重复配置也没副作用。默认勾选。
- 凭证页与自动配置里区分 `type=web` 与 `type=session` 两种 access token；web-only 号单给一个
  「换一把桌面 session」的动作。

### 变更

- **自动配置的次序调整：铸 `crsr_` Key 提到换 session 之前。** 趁手上这把 token（哪怕是会死的
  web token）还活着先把保命的 API Key 铸出来，之后任何一步把 token 弄掉了，查花费、进网关都还有
  退路。整条流水线现在是：铸 Key → 换 session → 开按需 → 开数据保留 → 刷用量。
...
- **Windows 上切号被误判成只读。** 那台机器只是登出了 Cursor，库里还留着
  `stripeMembershipType` 这类非身份键，于是被当成「登着号却缺 token」= 键名漂移，
  切号整块锁死。判据改成身份键（`cachedEmail` / `cachedUserId` / `cachedScopedProfile`），
  没人登着就照常放行写入。
- **安装目录「未检测到」但填了也没用。** 卸载后留下的空壳目录以前算「探到了」，
  手填一级填错也照单全收 —— 界面说一切正常，启动 Cursor 和打补丁到用的时候才失败。
  现在只认里面真有 Cursor（`product.json` 或 `Cursor.exe`）的目录，填了没生效会说
  「这个目录里没有 Cursor」。

### 变更

- **改 Cursor 目录不用再重启客户端。** 保存后切号、Sand、CRSR、网关一起指到新路径；
  切号或补丁正在跑时会拒绝（等它结束再改）。
- **那两行路径现在能浏览、能重新探测。** 系统文件夹选择器选完即生效；点进 `User` /
  `globalStorage`、指到 `Cursor.exe` 都会自动收到真正那一层；标题右边照实说
  「已找到 · 3.19.13」还是没找到。
- 账号页筛子上的数字改成「点下去会剩几个」——交叉计数，自己那一维跳过。

### 安全

- `rustls` 升到 0.23.45（RUSTSEC-2026-0285）。
