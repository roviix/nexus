...
//! `aiserver.v1.InferenceService/Stream`（以及 ChatGPT / Grok / Kiro 各自的后端）。号从用户自己的
//! 账号（`nexus-accounts` 等）或本机 Cursor 登录态来。网关本身**不改 Cursor 一个字节**。
//!
//! 客户端入口只有一种：讲 OpenAI / Anthropic 方言的标准客户端（Claude Code、Codex、SDK、curl）。
//! Cursor IDE 的 Agent 面板**不经过**网关——面板走 `agent.v1` 到 api5，客户端没有任何指向
//! 127.0.0.1 的口子；要让面板换号付账是补丁（`nexus-sand` / `nexus-crsr`）的事，与网关无关。
//! 早先另开过一条 h2c 透传口给 `cursor-agent` CLI 与 Sand 改道用，两条链路都没有端到端验收过，
//! 2026-09 整条拆掉：网关只干一件事——拿号池的号，对本机客户端讲标准方言。
//!
//! 这是对 ARCHITECTURE §1.2「不做本地网关」的有意反转；边界仍按 §3.4：只向下依赖
...
//!
//! ```text
//! service     编排：起停 / 设置 / 口令 / 状态（Tauri 只认它）
...
//!   ├─ channel    通道注册表：`{通道}/{模型}` 选路，默认通道由用户指定
//!   ├─ models     客户端模型名 → Cursor 模型名（不映射的话 Claude Code 直连必 400）+ force_model
//!   ├─ lane       号从哪来：RelayLane 额度接力 + Source（Cursor 登录号 / nexus-accounts 托管号 /
//!   │             订阅号——每条通道自己一条 lane）
//!   ├─ ledger     请求账本：每次请求记一行（账号 / 模型 / token / 耗时），概览的「本地用量」
...
//! ```
//!
//! 以上全部已落地并对真上游验证（`examples/probe.rs` / `serve.rs`）。
//! Tauri 侧在 `apps/desktop/src-tauri/src/commands/gateway.rs` 与 `pages/GatewayPage.tsx`。
//!
//! 协议知识移植自上游私有仓库里线上验证过的 JS 实现：纯函数的测试向量由它直接跑出来，
//! 消息映射拿它的编码器字节做跨实现对拍，不凭记忆、不凭"理解"重写。有意偏离它的地方只有两处，
//! 都在代码注释里说明了理由：上游 OUTPUT_TOKEN_LIMIT 在已有输出时算 `finish=length` 而不是报错；
