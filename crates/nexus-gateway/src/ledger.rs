...
//! 的响应。
//!
//! 早先透传口上的「IDE Agent 面板拦截」也往这张表写过行（`dialect = 'ide-agent'`）。那条路径
//! 已经拆掉；它的口径（Cursor 每一轮的模型调用，几十万 token 一行）和方言口的请求不是一回事，
//! 所以启动时 [`Ledger::prune`] 把残留的那些行一并清掉，汇总里不再出现。
...
/// 已拆掉的 IDE Agent 面板拦截曾借 `dialect` 列当来源标签写下的值。只在 [`Ledger::prune`] 里
/// 认它，用来清掉老库里的残留行。
const LEGACY_IDE_AGENT: &str = "ide-agent";
...
impl Ledger {
        ...
        self.db.with(|c| {
            let mut stmt = c.prepare(
                "SELECT account, COUNT(*), SUM(ok = 0), SUM(input_tokens + output_tokens)
                 ...
                 WHERE at_ms >= ?1 AND channel = ?2
                 GROUP BY account ORDER BY COUNT(*) DESC",
            )?;
            let rows = stmt.query_map(params![since_ms, channel], |r| {
                Ok(NamedUsage {
                    name: r.get(0)?,
                    calls: r.get(1)?,
                    errors: r.get(2)?,
    ...
    /// 裁掉 `KEEP_DAYS` 之前的行，顺手清掉已拆掉的 IDE 拦截留下的行。启动时跑一次。
    pub fn prune(&self) -> Result<usize> {
        let cutoff = now_ms() - KEEP_DAYS * DAY_MS;
        self.db.with(|c| {
            let expired = c.execute("DELETE FROM gateway_requests WHERE at_ms < ?1", [cutoff])?;
            let legacy = c.execute(
                "DELETE FROM gateway_requests WHERE dialect = ?1",
                [LEGACY_IDE_AGENT],
            )?;
            Ok(expired + legacy)
        })
    ...
    pub fn summary(&self, days: u32, tz_offset_min: i32) -> Result<UsageSummary> {
        self.summary_at(now_ms(), days, tz_offset_min)
    }

    fn summary_at(&self, now_ms: i64, days: u32, tz_offset_min: i32) -> Result<UsageSummary> {
        let days = days.clamp(1, KEEP_DAYS as u32) as i64;
        ...
        self.db.with(|c| {
                ...
                .collect();
            {
                let mut stmt = c.prepare(
                    "SELECT (at_ms + ?1) / 86400000 AS d,
                     ...
                     FROM gateway_requests WHERE at_ms >= ?2
                     GROUP BY d",
                )?;
            ...
            let today = totals(c, today_since_ms)?;
            let window = totals(c, since_ms)?;

            let by_model = {
                let mut stmt = c.prepare(
                    "SELECT model, COUNT(*), SUM(ok = 0), SUM(input_tokens + output_tokens)
                     FROM gateway_requests WHERE at_ms >= ?1
            ...
            let recent = {
                let mut stmt = c.prepare(
                    "SELECT at_ms, account, model, routed, ok, status, kind,
                     ...
                     FROM gateway_requests WHERE 1 = 1 ORDER BY id DESC LIMIT 8",
                )?;
            ...
            let first_at: Option<i64> = c
                .query_row(
                    "SELECT MIN(at_ms) FROM gateway_requests WHERE 1 = 1",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(None);
...
fn totals(c: &rusqlite::Connection, since_ms: i64) -> rusqlite::Result<Totals> {
    ...
    let median = |col: &str| -> rusqlite::Result<Option<i64>> {
        ...
        let n: i64 = c.query_row(
            &format!(
                "SELECT COUNT(*) FROM gateway_requests WHERE at_ms >= ?1 AND ok = 1 AND {col} IS NOT NULL"
            ),
            [since_ms],
            |r| r.get(0),
        )?;
        ...
        c.query_row(
            &format!(
                "SELECT {col} FROM gateway_requests WHERE at_ms >= ?1 AND ok = 1 AND {col} IS NOT NULL
...
mod tests {
    ...
    fn days_are_bucketed_in_the_users_timezone() {
        ...
        let s = l.summary_at(NOW, 7, UTC8).unwrap();
        ...
        let utc = l.summary_at(NOW, 7, 0).unwrap();
    ...
    fn totals_group_by_model_and_lowercased_account() {
        ...
        let s = l.summary_at(NOW, 7, 0).unwrap();
    ...
    /// 老库里 IDE 拦截留下的行在 prune 时清掉——那条路径已经不存在，它的口径也和方言口混不到一起。
    #[test]
    fn legacy_ide_agent_rows_are_pruned() {
        let l = ledger();
        l.record(ok_record("a@x.com", "gpt-5.6-sol", 10, 5));
        l.record(RequestRecord {
            dialect: LEGACY_IDE_AGENT,
            ..ok_record("a@x.com", "claude-opus-5", 400_000, 800)
        });
        assert_eq!(l.summary(7, 0).unwrap().window.calls, 2, "清之前两行都算");
        assert_eq!(l.prune().unwrap(), 1);
        let s = l.summary(7, 0).unwrap();
        assert_eq!(s.window.calls, 1);
        assert_eq!(s.by_model[0].name, "gpt-5.6-sol");
    }
